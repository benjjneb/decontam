...
We all know that contaminants are a problem in marker-gene and metagenomics data. Right now, it is widespread practice to addredd this by sequencing negative controls, and then ... ignoring them, in large part because there haven't been easy to use tools available to identify contaminants from this kind of data. That's what the `decontam` package is for: It provides a simple interface that takes in your table of sequence features (e.g. ASVs, OTUs, MAGs, genera, ...), and outputs the contaminants identified by simple statistical tests that are based on signatures demonstrated over many previous studies.
